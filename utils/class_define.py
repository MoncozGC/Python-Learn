...
class DatabaseConnectBean(object):
    ...
    def checkOutTime(self, timeout):
        if self.connect is None \
                or self.isUse is False \
                or (datetime.datetime.now() - self.lastTime).seconds > timeout:
            return True
        return False
    ...
    def run(self):
        while True:
            ...
            for dbmng in self.databaseManagerList:
                # print("%s>> %s" % (threading.current_thread().name,str(dbmng.flag)))
...
class DatabaseManagerBean(object):
    ...
    def __init__(self, databaseType, host, port, username, password, databaseName, databaseClearThread):
        if databaseType == 'mysql' or databaseType == 'clickHouse':
