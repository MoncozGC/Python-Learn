...
def get_data(page: int, oid: str):
    ...
    r.raise_for_status()
    return r.json()['datas']['replies'], r.json()['datas']['page']['count']
    ...
    def loop(pid, tab):
        ...
        for item in pointer:
            if pid == item[1]:
                ...
                # f.write(f'INSERT INTO `blbl_content`(`uid`, `uname`,`time`, `video_bv`,`content`) VALUES ({mid},"{name}","{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(ctime))}","{bv}","{message}");\n')
                f.write(f'({mid},"{name}","{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(ctime))}","{bv}","{message}");\n')
                loop(item[0], tab + 1)
...
if __name__ == '__main__':
    ...
    sleep_time = 7.1  # 访问网页间隔，防止IP被禁，若运行程序后出现无法访问评论区的现象，等待2小时即可

    # 记得在这里输入视频的BV号！
    bv = 'BV1FL411E7g3'

    oid = get_oid(bv)
    f = open(f'./datas/{time.strftime("%Y%m%d-%H%M")}.txt', 'w', encoding='utf-8')
    page = 1
    while True:
        try:
