...
def wr_Excel():
    ...
    cur = conn.cursor()
    sql = """SELECT  TOP 5 b.busno,a.wareid,a.purprice,saleprice,b.sumqty,sum(c.wareqty),sum(c.netprice),sum(c.purprice),sum(c.netprice)-sum(c.purprice),a.createtime,
    CASE WHEN a.purstatus=1  THEN '否' ELSE '是' END,'否'
    ...
    save_dir = current_dir + "File_" + "%s.xlsx" % nowDate
    book.save(save_dir)
