...
def canvas_get_number(_specify_colum, _compare_value, _judge):
    ...
    # print_ts(student_id[0][0:1])
    # print_ts(student_id[0][0:2])
    ...
    for x in range(2201, 2238):
        for i in stu_avg:
            if stu_avg.values[a] > 0:
                # print_ts(stu_avg.index[a])
                # print_ts(int(stu_avg.values[a]))
                # 拼接字符串
                # stre = str(stu_avg.index[a]) + "_" + str(int(stu_avg.values[a] * 100))
...
def page_simple_layout(_specify_colum, *_args):
    """
    将画布全部输出到一个文件中
    :param _specify_colum:
    :return:
    ...
    page.add(
        # canvas_get_number(_specify_colum, 72, 'out'),
        # canvas_get_avg(_specify_colum)
        bar,
        c,
    )
...
if __name__ == '__main__':
    ...
    # 需要返回: x值为班级, y1值为平均分, y2值为不及格率

    # 根据不及格率生成画布
    # class_id_out, out_number = canvas_get_number(specify_colum, 72, 'out')
    # 根据平均分生成画布
    # class_id_avg, avg_fra = canvas_get_avg(specify_colum)

    # 根据优秀人数生成画布
    # class_id_goods, goods_number = canvas_get_number(specify_colum, 108, 'goods')

    # print_ts("数据: ", class_id_out, out_number)
    # print_ts("数据: ", class_id_avg, avg_fra)

    # bar = (
    #     Bar(init_opts=opts.InitOpts(width="2150px"))
    #         .add_xaxis(class_id_avg)
    #         .add_yaxis("不及格人数", out_number)
    #         .add_yaxis("优秀人数(大于108分)", goods_number)
    #         .set_global_opts(
    #         xaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts(rotate=-15)),
    #         title_opts=opts.TitleOpts(title="数据对比", subtitle="不及格人数和优秀人数对比"),
    #     )
    # )
    # bar.render("bar_rotate_xaxis_label.html")

    pass_ids, pass_number, pass_rate = canvas_get_rate(specify_colum, 72, 'pass')
    goods_ids, goods_number, goods_rate = canvas_get_rate(specify_colum, 108, 'goods')
    out_ids, out_number, out_rate = canvas_get_rate(specify_colum, 72, 'out')
    ...
    # c.render("bar_rotate_xaxis_label.html")

    # page_simple_layout(specify_colum, bar, c)
